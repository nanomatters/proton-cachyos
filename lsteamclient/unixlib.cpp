...
struct overlay_client_search
{
    const char *name;
    char path[PATH_MAX];
};

static pthread_mutex_t overlay_client_mutex = PTHREAD_MUTEX_INITIALIZER;
static void *overlay_client_module;
static void (*p_overlay_client_set_active)( int active );

static int find_overlay_client_module( struct dl_phdr_info *info, size_t size, void *arg )
{
    struct overlay_client_search *search = (struct overlay_client_search *)arg;
    const char *name;
...
{
    struct overlay_client_search search = {};
#if defined(__x86_64__)
    search.name = "libVkLayer_WINELAND_translate_x86_64.so";
#elif defined(__i386__)
    search.name = "libVkLayer_WINELAND_translate_i386.so";
#endif
    void (*set_active)( int active );

    pthread_mutex_lock( &overlay_client_mutex );
    if (!overlay_client_module && search.name &&
        dl_iterate_phdr( find_overlay_client_module, &search ))
        overlay_client_module = dlopen( search.path, RTLD_NOW | RTLD_LOCAL | RTLD_NOLOAD );
    if (!p_overlay_client_set_active && overlay_client_module)
        p_overlay_client_set_active = (void (*)(int))dlsym(
                overlay_client_module, "__wineland_overlay_client_set_active_v1" );
    set_active = p_overlay_client_set_active;
    pthread_mutex_unlock( &overlay_client_mutex );

    if (set_active) set_active( active );
}
